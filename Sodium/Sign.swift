...
extension Sign {
    public func extractEd25519PublicKeyFromEd25519SecretKey(ed25519SecretKey: Bytes) -> Bytes? {
        guard ed25519SecretKey.count == SecretKeyBytes else {
            return nil
        }
        
        var buffer = Array<UInt8>(count: PublicKeyBytes)
        guard .SUCCESS == crypto_sign_ed25519_sk_to_pk(&buffer, ed25519SecretKey).exitCode else { return nil }
        
        return buffer
    }
    
    public func signDetached(message: Bytes, key: Bytes) -> Bytes? {
        guard key.count == SecretKeyBytes else {
            return nil
        }
        
        var signature = Array<UInt8>(count: SecretKeyBytes)
        var signatureLength: UInt64 = 0
        
        guard .SUCCESS == crypto_sign_detached(&signature, &signatureLength, message, UInt64(message.count), key).exitCode else { return nil }
